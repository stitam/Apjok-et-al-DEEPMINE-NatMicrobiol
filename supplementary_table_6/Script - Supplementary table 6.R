...
#A) BLASTX
blastx__first_run <- read.delim(
  "../run1/results/Nanopore sequences Run1 blastx.tsv",
  stringsAsFactors = FALSE
)
...
#B) PRODIGAL
prodigal_first_run <- read.delim(
  "../run1/results/Nanopore sequences Run1 prodigal.tsv")
...
hgt_first_run <- read.delim(
  "../run1/results/Nanopore sequences Run1 mobile gene pool.tsv")
...
#merge with annotations and mobile gene pool network
#A) BLASTX
blastx__second_run <- read.delim(
  "../run2/results/Nanopore sequences Run2 blastx.tsv",
  stringsAsFactors = FALSE
)
...
#B) PRODIGAL
prodigal_second_run <- read.delim(
  "../run2/results/Nanopore sequences Run2 prodigal.tsv")
...
#C) horizontal gene transfer annotation based on mobile gene pool network
hgt_second_run <- read.delim(
  "../run2/results/Nanopore sequences Run2 mobile gene pool.tsv")
...
##4. REMOVE DUPLICATES OF UPSTREAM AND DOWNSTEAM BARCODES TO REDUCE SEQUENCE REUNDANCY
...
illumina<- read.csv("../run1/results/D9JY2_bcnumbers.csv", sep = "\t")
...
second_illumina<- read.csv("../run2/results/G73VD_bcnumbers.csv", sep = "\t")
...
# ORIGINAL THRESHOLDS AT SUBMISSION illu > 8, nano > 9
##7. APPLY TRESHOLDS FOR ILLUMINA AND NANOPORE
all_thr_illumina_nano<- illumina_nano_all[illumina_nano_all$Illumina.Read.Count>8 & illumina_nano_all$Nanopore.Read.Count>9,]
...
#uncomment lines below to print the plot to console
#cor.test(illumina_nano_all$log.Normalise.Nanopore, illumina_nano_all$log.Normalise.Illumina, method = "spearman")
#grob1 = grobTree(textGrob(paste("Before Thr: R = 0.26***"), x = 0.67, y = 0.99, gp = gpar(fontsize = 18, fontface = "bold")))
...
#plot1 +labs(x = "Nanopore Read Count (log scale)", y= "Illumina Read Count (log scale)") + theme_bw() +annotation_custom(grob1) +
...
#uncomment lines below to print the plot to console
#cor.test(all_thr_illumina_nano$log.Normalise.Nanopore, all_thr_illumina_nano$log.Normalise.Illumina, method = "spearman")
#grob3 = grobTree(textGrob(paste("After Thr: R = 0.34***"), x = 0.67, y = 0.99, gp = gpar(fontsize = 18, fontface = "bold")))
#ggplot(all_thr_illumina_nano, aes(log.Normalise.Nanopore, log.Normalise.Illumina)) + geom_point(color="#CC3333",size = 1) + geom_smooth(method = "lm", se=FALSE, colour="#555555", size=2)+
...
capillary <- read.csv("Information on Antibiotics and CFU values in vitro.csv")
...
genes_correct <- read.delim("CORRECTED Check gene mechanism and classification.tsv")
...
clusters <- read.csv(
  "Clustering of ARGs into Cluster IDs/Clustering of ARGs into Cluster IDs.csv",
  header=FALSE, 
  sep="\t"
)
...
hgt_IDs <- read.csv(
  "../databases/Mobile Gene Pool - Species per Event.tsv", sep="\t")
...
plasmid <- read.csv(
  "Plasmid Homologues/Plasmid Homologues.csv")
...
write.table(metagenome_final1, "Supplementary table 6.tsv", row.names = FALSE, sep="\t")
