...
def hatch_process(target,lista,*constants):
	if len(constants)>0 and constants[-1] == "TEST":
		for elem in lista:
			target(elem,*constants[:-1])
	else:
		execute([mp.Process(target=target,args=((elem,)+constants)) for elem in lista])
...
def demux():
	...
	execute([mp.Process(target=demux_fq,args=(config['data']['read1'],True,bcdict,)),mp.Process(target=demux_fq,args=(config['data']['read2'],False,bcdict,))])
...
if __name__=="__main__":
	...
	for amplicon in amplicons:
		hatch_process(check_amplicon,samples,amplicon)
	#Reference mapping
